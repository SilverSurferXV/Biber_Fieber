...
import { Search, ArrowUpDown, ArrowUp, ArrowDown, Trash2, Download, Upload } from 'lucide-react';
import { useDebounce } from '../helpers/useDebounce';
import { postAdminCustomersDeleteAll } from '../endpoints/admin/customers/delete-all_POST.schema';
import { getAdminCustomersBackup } from '../endpoints/admin/customers/backup_GET.schema';
import { postAdminCustomersRestore } from '../endpoints/admin/customers/restore_POST.schema';
import { useMutation } from '@tanstack/react-query';
import { toast } from 'sonner';
import { AdminCustomerRow } from './AdminCustomerRow';
import { AdminCustomerCreateForm } from './AdminCustomerCreateForm';
import { BackupChoiceDialog } from './BackupChoiceDialog';
import styles from './AdminCustomers.module.css';

type SortColumn = 'name' | 'plz' | 'email' | 'points' | 'bibercode' | null;
...
export const AdminCustomers = () => {
  ...
  const { data } = useAdminCustomers({ page, search: debouncedSearch || undefined, excludeBusiness: true });
  const customers = data?.customers || [];
  
  const [backupDialogOpen, setBackupDialogOpen] = useState(false);
  const { data: allCustomersData } = useAdminCustomers({ page: 1, limit: 10000, excludeBusiness: true });
  ...
  const handleBackupAll = async () => {
    try {
      ...
      const blob = await getAdminCustomersBackup();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `biber-fieber-backup-${new Date().toISOString().split('T')[0]}.json`;
      a.click();
    ...
    try {
      setBackupLoading(true);
      const blob = await getAdminCustomersBackup({ userIds });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      const now = new Date();
      ...
      if (userIds.length === 1) {
        ...
        const name = c ? `${c.firstName || ''}_${c.lastName || ''}`.trim().replace(/\s+/g, '-') : 'kunde';
        a.download = `${dateStr}_${name}.json`;
      } else {
        a.download = `biber-fieber-backup-${now.toISOString().split('T')[0]}.json`;
      }
      a.click();
      URL.revokeObjectURL(url);
      toast.success("Backup erfolgreich erstellt");
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      const data = await postAdminCustomersRestore(restoreFile);
  ...
  const deleteMutation = useMutation({
    mutationFn: (password: string) => postAdminCustomersDeleteAll({ password }),
    onSuccess: (data) => {
    ...
    <div className={styles.viewContainer}>
      <div className={styles.header}>
        ...
        <div className={styles.headerActions}>
          ...
          <Button onClick={() => setCreateOpen(true)}>Kunde anlegen</Button>
            ...
            {plzs.map(plz => (
              <React.Fragment key={plz}>
                <tr className={styles.groupHeader}>
                  <td colSpan={7}>PLZ: {plz}{getCityForPlz(plz) ? ` — ${getCityForPlz(plz)}` : ''}</td>
                </tr>
                {(grouped[plz] || []).map((c: any) => (
                  <AdminCustomerRow key={c.id} customer={c} />
                ))}
              </React.Fragment>
            ))}
          </tbody>
