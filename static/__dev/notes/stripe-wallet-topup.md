...
## Architectural Decision 25.08.2026: Removal of Browser Handoff Fallback

- The browser-handoff fallback for Apple Pay / Google Pay has been **removed** from the frontend (`components/TopupPaymentDialog`). 
- Native wallets now work exclusively through the native `@capacitor-community/stripe` plugin. 
- When no wallet is available in the native app, no wallet section is rendered at all, and the user must fall back to Klarna/Sofort/PayPal/credit card.
- **WARNING FOR FUTURE TURNS:** The backend handoff infrastructure was deliberately **KEPT** even though it is now dead code from the current frontend's perspective. 
  - Kept files include: endpoints `wallet/handoff/create_POST`, `create-intent_POST`, `confirm_POST`, `info_GET`, plus `pages/aufladen.$token`, `components/HandoffTopupCheckout`, and `helpers/useTopupHandoff`. 
  - **Reason:** The native app ships a LOCAL bundle that does not update on Floot publishes. The app currently installed on users' devices still calls these endpoints for wallet top-ups. Deleting them before a new native build is rolled out would immediately break top-ups for existing users. They may only be removed once the new build (with the plugin and ideally the remote-bundle switch) is confirmed live.
- Native wallet buttons now use `components/NativeWalletButton` with official Apple Pay / Google Pay branding, because plain text buttons are an App Store review risk. The Apple/Google "Pay" wordmark must stay untranslated.
- The plugin API usage in `helpers/nativeStripeWallet` was verified against the real v8 plugin interfaces and is correct: 
  - `createApplePay` requires `paymentIntentClientSecret`, `paymentSummaryItems` (amount in MAJOR units, i.e., euros not cents), `merchantIdentifier`, `countryCode`, and `currency`. 
  - `createGooglePay` needs only `paymentIntentClientSecret` natively, as the other fields are web-only. 
  - The result strings are `applePayCompleted`/`applePayCanceled`/`applePayFailed` and `googlePayCompleted`/`googlePayCanceled`/`googlePayFailed`.

## BEWEIS 25.08.2026: App auf dem Gerät läuft veraltetes lokales Bundle

- **Prod log evidence from a user test at ~14:18 on 2026-08-25:**
  - `[native-probe]` on `POST /_api/wallet/handoff/create` reported `origin: 'capacitor://localhost'` → the installed app serves a LOCAL bundle (Option C), NOT the Option-A remote bundle. UA was `Macintosh; Intel Mac OS X 10_15_7 AppleWebKit/605.1.15` without Safari/Version token = iOS WKWebView, so the test ran on iOS, not Android.
  - The `nativeStripeWallet` telemetry beacon ("native wallet availability") produced ZERO hits in prod logs because the shipped build used a RELATIVE `/_api/` URL, which resolved to the local Capacitor server and never reached the backend. Its absence proves nothing about the JS version.
  - What IS proven: The app serves a local bundle (`capacitor://localhost`), and the handoff button was displayed, meaning `getAvailability` fell through. Since Capacitor 8 is confirmed in the Codemagic pipeline (user-confirmed) and the test was on iOS, the leading hypothesis is now a missing Apple Pay entitlement or a Merchant ID mismatch rather than a missing plugin. An entitlement failure makes `isApplePayAvailable()` throw, simulating a missing plugin.
  - **Conclusion:** No device telemetry is obtainable until a new native build ships the fixed absolute-URL beacon, because the local bundle does not update on Floot publishes.
- **Also observed:** API calls from the native app DO reach prod successfully via the Authorization bearer fallback (`hasCookieHeader: false, hasAuthHeader: true, outcome: 'ok-bearer'`). This contradicts the older claim in `docs/native-app-updates-ohne-review.md` that Option C is completely broken by CORS — the bearer fallback in apiFetchGuard works.
- **Also observed:** the browser handoff path works correctly and Apple Pay IS available in Safari on the published domain (`applePaySessionExists: true, canMakePayments: true`, origin `https://biberfieber.floot.app`, `pk_live_`). So the fallback is functioning as designed; it is simply still being triggered.
- **Diagnostics gotcha (fixed 25.08.2026):** `navigator.sendBeacon` does NOT go through the patched `window.fetch` in `apiFetchGuard`, so it gets no URL rewriting. Any beacon must use the absolute `PUBLISHED_ORIGIN` constant, never a relative `/_api/` path, or it dies on the local Capacitor server in the native app.
- **Required next step (outside Floot):** a Codemagic build that installs `@capacitor-community/stripe` (needs `@capacitor/core >= 8`) and runs `cap sync`. Strongly recommended to combine it with the Option-A switch (`server.url = https://biberfieber.floot.app`) in the SAME build, so future frontend iterations go live instantly without further reviews.

## ROOT CAUSE FOUND (supersedes the device-specific conclusions below)

- The root cause of "Apple Pay / Google Pay never appears in the native app" initially appeared to be a completely missing `@capacitor-community/stripe` npm package. However, with Capacitor 8 confirmed in the pipeline, the leading suspect for iOS is that the plugin is present but Apple Pay signing is misconfigured (missing `com.apple.developer.in-app-payments` entitlement, capability missing in the provisioning profile, or mismatched merchant ID). Any of these causes `isApplePayAvailable()` to throw, forcing the fragile browser handoff.
- The npm package `@capacitor-community/stripe` (v8.2.1) is now installed in Floot. A NEW NATIVE BUILD is strictly required to take effect, as Floot publishes do not update the local native bundle.
