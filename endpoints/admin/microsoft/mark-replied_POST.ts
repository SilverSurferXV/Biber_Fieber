import { schema, OutputType } from "./mark-replied_POST.schema";
...
export async function handle(request: Request) {
  try {
    ...
    const msResponse = await fetch(graphUrl, {
      ...
      body: JSON.stringify({ categories: ["Beantwortet"] }),
    });

    const responseText = await msResponse.text();

    if (!msResponse.ok) {
      throw new Error(`Failed to mark email as replied: ${responseText}`);
    }
