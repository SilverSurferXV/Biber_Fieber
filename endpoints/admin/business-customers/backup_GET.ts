...
export async function handle(request: Request) {
  try {
    ...
    if (user.role !== "admin") {
      console.error("Non-admin user attempted to backup business customers");
     ...
     // 1. Query all non-admin business users
     ...
     if (users.length === 0) {
      ...
      return new Response(JSON.stringify(emptyBackup), {
        headers: {
          ...
          "Content-Disposition": `attachment; filename="biber-fieber-firmenkunden-backup-${new Date().toISOString().split('T')[0]}.json"`,
        },
      });
    }

    const businessUserIds = users.map((u) => u.id);
    ...
    for (let i = 0; i < businessUserIds.length; i += 5000) {
      const chunk = businessUserIds.slice(i, i + 5000);
    ...
    // Deduplicate records where multiple relationships might have fetched the same record
    ...
    console.error("Error generating business customers backup:", error);
    return new Response(
